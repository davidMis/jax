...
def _can_cast_weak_type(a: jnp.dtype, b: jnp.dtype) -> bool:
  try:
    return jnp.result_type(a, b) in (a, b)
  except ValueError:
    return False
...
class _Extern:
  ...
  def matches(self, avals: Sequence[jax_core.ShapedArray]) -> bool:
    ...
    return all(
        aval.dtype == arg_type
        or (aval.weak_type and _can_cast_weak_type(aval.dtype, arg_type))
...
def _make_dispatch_table(
  ...
  def inner(ctx: LoweringRuleContext, *args: ir.Value) -> ir.Value:
    ...
    for aval, arg, arg_type in zip(ctx.avals_in, args, h.arg_types):
      ...
      if aval.weak_type and aval.dtype != arg_type:
...
def _argreduce_lowering(
  ...
  ctx = ctx.replace(
      avals_in=[a_aval, a_aval.update(dtype=jnp.dtype(jnp.int32))]
  )
